...
class my_sqwBrownian_Diffusion(mapi.IFunction1D):
    ...
    def function1D(self, xvals):
        ...
        return QENSmodels.sqwBrownianTranslationalDiffusion(xvals, q, scale=scale, center=center,
                                                            D=D)
...
""" Below is the template string to create individual model for each spectrum.
    ...
    A similar string can be obtained after defining the fitting model on Mantid Workbench fit 
    wizard: 

    "Setup" --> "Manage Settup" --> "Copy to Clipboard". This action will save the model
    as a string which you can later paste onto this script.

    Our initial guesses are scale=7., center= 0. and D=2. 
"""

single_model_template = """name=my_sqwBrownian_Diffusion,$domains=i,scale=7.,center=0.,D=2.,Q=_Q_"""
...
for wi in range(selected_wi):
    single_model = single_model_template.replace("_Q_", str(Q[wi]))  # insert Q-value
...
for wi in range(selected_wi):
    if wi == 0:
        domain_model.update({"InputWorkspace": 'QENS_data', "WorkspaceIndex": str(wi),
                             "StartX": str(-4.95), "EndX": str(4.95)})
    else:
        domain_model.update(
            {"InputWorkspace_" + str(wi): 'QENS_data', "WorkspaceIndex_" + str(wi): str(wi),
             "StartX_" + str(wi): str(minE), "EndX_" + str(wi): str(maxE)})
...
for i in range(4):
    print(f'Workspace {i}:')
    print(f'scale: {paramTable.column(1)[3 * i]:.2f}+/-{paramTable.column(2)[3 * i]:.2e}')
    print(f'center: {paramTable.column(1)[3 * i + 1]:.2f}+/-{paramTable.column(2)[3 * i + 1]:.2e}')
    print(f'D: {paramTable.column(1)[3 * i + 2]:.2f}+/-{paramTable.column(2)[3 * i + 2]:.2e}')
...
for indx, item in enumerate(mapi.mtd['fit_Workspaces']):
    ax[indices_plot[indx][0], indices_plot[indx][1]].grid()
    ax[indices_plot[indx][0], indices_plot[indx][1]].plot(item.readX(0), item.readY(0), label='exp')
    ax[indices_plot[indx][0], indices_plot[indx][1]].plot(item.readX(1), item.readY(1),
                                                          label='calc')
    ax[indices_plot[indx][0], indices_plot[indx][1]].plot(item.readX(2), item.readY(2),
                                                          label='diff')

ax[indices_plot[indx][0], indices_plot[indx][1]].legend()
fig.show()
