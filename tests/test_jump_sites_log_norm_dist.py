...
class TestJumpsSitesLogNorm(unittest.TestCase):
    ...
    def test_type_size_hwhm_jump_sites_log_norm_q_nb(self):
        ...
        hwhm_equiv, eisf_equiv, qisf_equiv = QENSmodels.hwhmEquivalentSitesCircle(1.)
        self.assertEqual(eisf[0], eisf_equiv[0])

        self.assertSequenceEqual(numpy.round(qisf[0, 0], 3).tolist(),
                                 [0.001, 0.002, 0.003, 0.004, 0.005, 0.007, 0.009, 0.01, 0.011,
                                  0.012, 0.013, 0.012, 0.011, 0.01, 0.009, 0.007, 0.005, 0.004,
                                  0.003, 0.002, 0.001])
    ...
    def test_type_size_hwhm_jump_sites_log_norm_q_array(self):
        ...
        numpy.testing.assert_array_equal(numpy.round(hwhm1[0, 1], 3),
                                         [0.171, 0.19, 0.212, 0.236, 0.263, 0.292, 0.326, 0.362,
                                          0.403, 0.449, 0.5, 0.557, 0.62, 0.69, 0.768, 0.855,
                                          0.952, 1.06, 1.18, 1.313, 1.462])

        numpy.testing.assert_array_equal(numpy.round(eisf1, 3), [0.713, 0.256])
        # eisf should be the same as in hwhmEquivalentSitesCircle
        hwhm_equiv, eisf_equiv, qisf_equiv = \
            QENSmodels.hwhmEquivalentSitesCircle(q_input, Nsites=6, radius=1.0, resTime=1.0)

        self.assertSequenceEqual(eisf1.tolist(), eisf_equiv.tolist())

        numpy.testing.assert_array_equal(numpy.round(qisf1[1, 1], 3),
                                         [0.001, 0.001, 0.001, 0.002, 0.003, 0.004, 0.005, 0.005,
                                          0.006, 0.006, 0.007, 0.006, 0.006, 0.005, 0.005, 0.004,
    ...
    def test_raised_error_negative_coeffs(self):
        ...
        # N < 2
        self.assertRaises(ValueError, QENSmodels.hwhmJumpSitesLogNormDist, 1, 1, 1, 1, 1)
        # radius < 0
        self.assertRaises(ValueError, QENSmodels.hwhmJumpSitesLogNormDist, 1, 4, -1, 1, 1)
        # resTime < 0
        self.assertRaises(ValueError, QENSmodels.hwhmJumpSitesLogNormDist, 1, 4, 1, -1, 1)
        # sigma <= 0
        self.assertRaises(ValueError, QENSmodels.hwhmJumpSitesLogNormDist, 1, 4, 1, 1, -0.5)

    def test_raised_error_no_q_input(self):
        """ test that an error is raised if no values of q are given as input """
        self.assertRaises(TypeError, QENSmodels.sqwJumpSitesLogNormDist, 1)

    def test_type_sqw_jump_sites_log_norm(self):
        """ Test type of output """
        # w, q are floats
        self.assertIsInstance(QENSmodels.sqwJumpSitesLogNormDist(1, 1), numpy.ndarray)
        # w, q are vectors
        output = QENSmodels.sqwJumpSitesLogNormDist([1, 2, 3], [0.3, 0.4])
