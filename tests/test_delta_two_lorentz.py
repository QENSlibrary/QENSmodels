...
class TestDeltaTwoLorentz(unittest.TestCase):
    ...
    def test_type_output(self):
        """ Test type of output """
        self.assertIsInstance(QENSmodels.sqwDeltaTwoLorentz(1, 1), numpy.ndarray)

    def test_size_output(self):
        """ Test size of output """
        w_input = [1, 2, 3]
        q_input = [0.05, 0.3]
        sqw = QENSmodels.sqwDeltaTwoLorentz(w_input, q_input, 0.5, 2, [0.75, 0.5], [1, 2],
                                            [0.05, 0.04], [0.02, 0.03])
    ...
    def test_raised_exception(self):
        ...
        self.assertRaises(IndexError, QENSmodels.sqwDeltaTwoLorentz, w, q, 1, 0, [1, 1], [1, 1],
                          [1, 1], [1, 1])

    def test_raised_error_no_q_input(self):
        """ test that an error is raised if no values of q are given as input """
        self.assertRaises(TypeError, QENSmodels.sqwDeltaTwoLorentz, 1)
    ...
    def test_reference_data(self):
        ...
        q = 0.7
        output = numpy.column_stack([w, QENSmodels.sqwDeltaTwoLorentz(w, q,
                                                                      scale=1.,
                                                                      center=0,
                                                                      A0=0.01,
                                                                      A1=0.4,
